...
class TestEvaluate(AutoresearchTestBase):
    ...
    def test_policy_pareto_gate(self):
        champ = json.dumps({"accuracy": 0.70, "retrieval_s": 2.0, "deal_safety": 100.0, "n": 30})
        better = json.dumps({"accuracy": 0.72, "retrieval_s": 1.5, "deal_safety": 100.0, "n": 30})
        worse = json.dumps({"accuracy": 0.71, "retrieval_s": 5.0, "deal_safety": 90.0, "n": 30})
        merge = self.run_helper("evaluate", "score-policy", better, champ, "--min-cases", "15")
        revert = self.run_helper("evaluate", "score-policy", worse, champ, "--min-cases", "15")
        self.assertEqual(merge["decision"], "MERGE")
