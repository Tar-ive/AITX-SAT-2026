...
class GPUDealTask(vf.Task[GPUDealData]):
    ...
    async def decision_quality(self, trace: vf.Trace) -> float:
        ...
        violated = bool(platform and any(item in platform or platform in item for item in avoided))
        ...
        if platform and any(option in platform or platform in option for option in options):
            score += 0.3

        expected_condition = str(truth.get("condition", "")).lower()
        actual_condition = str(prediction.get("condition", "")).lower()
        if expected_condition and actual_condition and (
            expected_condition in actual_condition or actual_condition in expected_condition
        ):
            score += 0.2
        ...
        if max_lead_time is not None:
            try:
                score += 0.1 if int(prediction.get("lead_time_days", 10**6)) <= int(max_lead_time) else 0.0
